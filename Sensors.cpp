...
void performSelfTest() {
  ...
  float vs = readSupplyVoltage();
  if (vs < 9.0f) { Serial.print("Low Supply: "); Serial.println(vs); pass = false; }

  float vb = readBatteryVoltage();
  if (vb < 3.2f) { Serial.print("Low Battery: "); Serial.println(vb); pass = false; }
