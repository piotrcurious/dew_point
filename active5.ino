...
void controlCoolingPWM(int pwmValue) {
  analogWrite(coolerPin, constrain(pwmValue, minPWM, maxPWM)); // Constrain PWM to valid range
}
...
void createCoolingProfile(float estimatedDewPoint) {
  ...
  for (int i = 0; i < totalDataPoints; i++) {
    ...
    delay(500); // Allow the cooler to adjust

    // Read sensor values, stabilize before reading
    float temperature = bme.readTemperature();
    float humidity = bme.readHumidity();
    float pressure = bme.readPressure() / 100.0F; // Convert to hPa

    // Calculate and adjust dew point
...
void verifyDewPointWithHeatingProfile() {
  ...
  sht4x.setHeater(SHT4X_HEATER_MEDIUM, 10); // Medium heating for 10 milliseconds to minimize temperature effect
  delay(50); // Allow some time to observe immediate response
  ...
  // Deactivate the SHT4x heater after the verification phase
  sht4x.setHeater(SHT4X_HEATER_OFF, 0);
}

// Polynomial curve fitting function (using a least-squares approach for better accuracy)
float fitPolynomialCurve(float *x, float *y, int n) {
  // Implement a basic polynomial fitting algorithm here
  // Example uses a simplified 2nd degree polynomial fitting
  ...
  float a = (sumY * (sumX2 * sumX4 - sumX3 * sumX3) - sumX * (sumXY * sumX4 - sumX2Y * sumX3) + sumX2 * (sumXY * sumX3 - sumX2Y * sumX2)) / denominator;
  float b = (n * (sumXY * sumX4 - sumX2Y * sumX3) - sumY * (sumX * sumX4 - sumX2 * sumX3) + sumX2 * (sumX * sumX2Y - sumXY * sumX2)) / denominator;
...
void monteCarloSimulation(float *empiricalTemperatures, float *empiricalDewPoints, int n) {
  ...
  // Iterate over possible deviations of gas concentrations
  for (float co2Deviation = 0.9; co2Deviation <= 1.1; co2Deviation += 0.005) {
    for (float so2Deviation = 0.9; so2Deviation <= 1.1; so2Deviation += 0.005) {
      for (float no2Deviation = 0.9; no2Deviation <= 1.1; no2Deviation += 0.005) {
...
void loop() {
  delay(5000); // Main loop delay to prevent continuous execution
}
