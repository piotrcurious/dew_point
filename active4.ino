...
void verifyDewPointWithHeatingProfile() {
  ...
  sht4x.setHeater(SHT4X_HEATER_MEDIUM, 50); // Medium heating for 50 milliseconds to minimize temperature effect
  delay(100); // Allow some time to observe immediate response
...
float fitPolynomialCurve(float *x, float *y, int n) {
  ...
  sumError += abs(y[i] - estimatedY); // Calculate the fitting error
  }
  return sumError;
}
  ...
  for (float co2Deviation = 0.9; co2Deviation <= 1.1; co2Deviation += 0.005) {
    for (float so2Deviation = 0.9; so2Deviation <= 1.1; so2Deviation += 0.005) {
      for (float no2Deviation = 0.9; no2Deviation <= 1.1; no2Deviation += 0.005) {
        ...
        // Compare the hypothetical curve to the empirical curve using curve fitting
